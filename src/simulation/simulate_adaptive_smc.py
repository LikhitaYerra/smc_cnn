...
def run_adaptive_smc_simulation(
    scenario_name: str,
    ...
    predictor = CNNEnvironmentPredictor(
        model_path="models/cnn_environment_classifier.pt",
    )

    map_generator = EnvironmentMapGenerator(
        map_size=64,
        seed=42,
    )

    env_map = map_generator.generate(scenario_name)
    prediction = predictor.predict(env_map)
    predicted_scenario = prediction["scenario"]

    initial_params = get_adaptive_smc_parameters(predicted_scenario)
    ...
    for i in range(len(t)):
        ...
        # In this first adaptive version, we predict the scenario every 1 second.
        # Later, this can be replaced with real sensor maps.
        if i % int(1.0 / dt) == 0:
            env_map = map_generator.generate(scenario_name)
            prediction = predictor.predict(env_map)
            predicted_scenario = prediction["scenario"]
            new_params = get_adaptive_smc_parameters(predicted_scenario)
            active_params = controller.update_parameters(new_params)
        else:
            active_params = controller.parameter_filter.previous_params
        ...
        log_data.append(
            {
                ...
                "lambda_x": active_params["lambda_x"],
                "lambda_y": active_params["lambda_y"],
                "lambda_theta": active_params["lambda_theta"],
                "k_v": active_params["k_v"],
                "k_omega": active_params["k_omega"],
                "phi": active_params["phi"],
                "omega_smoothing": active_params["omega_smoothing"],
    ...
    plot_dir = f"results/plots/adaptive_smc/{scenario_name}"
    log_dir = "results/logs/adaptive_smc"
    ...
    plot_robot_path(
        xs,
        ys,
        title=f"Adaptive CNN-SMC Path - {scenario_name}",
    ...
    plot_tracking_error(
        t,
        errors,
        title=f"Adaptive CNN-SMC Tracking Error - {scenario_name}",
    ...
    plot_control_signals(
        ...
        v_values,
        omega_values,
        title=f"Adaptive CNN-SMC Control Signals - {scenario_name}",
...
def main():
    ...
    for scenario in scenarios:
        run_adaptive_smc_simulation(**scenario)
